...
exports.get_post_by_id = [
  ...
  param('id').escape().trim().isLength({min:24}).isLength({max:24}).withMessage("post not found"),
    ...
    try {
      ...
      let results = await async.parallel({
        ...
        Comments: function(callback) {
          Comment.find({'post': req.params.id}, 'formatted_date message name timestamp')
            .sort({timestamp:-1})
            .exec(callback);
        }
      });
      ...
      res.status(200).send({post:results.Post, date: results.Post.date, comments: results.Comments})
    } catch (error) {
...
exports.post_comment = [
  ...
  param('id').trim().escape('bad params').isLength({min:24}).withMessage('invalid'),
  param('id', "there is no post here").custom(async(val) =>{
    try {
      let post = await Post.findById(val).exec();
      if(post === null){
  ...
  async (req,res) =>{
    try {
      ...
      let comments = await Comment.find({'post':req.params.id},'formatted_date message name timestamp')
      .sort({'timestamp':-1})
      .exec()
